...
if res_post.status_code == 201:
    order_id = res_post.json()['id']
    print(f"Advancing Order {order_id} to AWAITING_PAYMENT...")
    advance_url = f"http://127.0.0.1:8000/api/orders/{order_id}/advance/"
    advance_data = {
        "status": "AWAITING_PAYMENT",
        "notes": "Order placed, awaiting offline payment proof."
    }
    res_adv = requests.post(advance_url, headers=headers, json=advance_data)
    print(res_adv.status_code, res_adv.text)
